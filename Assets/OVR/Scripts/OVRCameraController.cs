...
public class OVRCameraController : MonoBehaviour
{
	...
	private const int EyeResolution = 1024;	// resolution of eye textures
	public int EyeSamples = 2;				// multisamples for eye textures
	public int EyeDepth = 24;				// depth of each eye texture in bits
	// 										// As a minor optimization, depth precision may be set to 16 bits.
	//// -- UnityAndroid
    ...
    private const float DEFAULT_EYE_HEIGHT = 1.675f;
	...
	public Vector3 		NeckPosition       = new Vector3(0.0f, DEFAULT_EYE_HEIGHT - 0.15f - 1.0f,  0.0f);
	//// -- UnityAndroid
	...
	// Callback setup
	public event Action OnConfigureVrModeParms = null;
	public event Action OnApplicationResumed = null;
	...
	IEnumerator OnResume()
	{
		...
		for ( int i = 0; i < EyeBufferCount * 2; i++ )
		{
			//	OVRDebugUtils.Assert( CameraTextures[i] != null );
			if ( ( CameraTextures[i] != null ) && !CameraTextures[i].IsCreated() )
			{
	...
	void Awake()
	{
		...
		// WORKAROUND: There are cases where OnApplicationFocus() is called for a scene which
		// is destroyed before OnResume() has finished. In that case, EnterVrMode is never called
		// and the result is a black screen. Record if we've not finished entering vr mode and
		// re-initiate for the next scene load.
		// NOTE: This will be fixed in the new integration with a static OVRManager handling
		// OnApplicationFocus() and OnApplicationPause().
		if ( pendingEnter )
		{
			Debug.Log( "PENDING ENTER!!!" );
			StartCoroutine( OnResume() );
		}
#endif	
	}
	...
	{		
...
#else
		// Configure app-specific vr mode parms such as clock frequencies - this needs to
		// happen before the plugin and eye render textures are initialized.
		...
		for ( int i = 0; i < EyeBufferCount * 2; i++ )
		{
			if ( ( CameraTextures[i] == null ) || !CameraTextures[i].IsCreated() )
			{
				...
				CameraTextures[i] = new RenderTexture(EyeResolution, EyeResolution, EyeDepth);
				CameraTextures[i].antiAliasing = EyeSamples;
				CameraTextures[i].Create(); // upload to the GPU so we can get the hardware id
				// Note: calling GetNativeTexturePtr() with mt-rendering will synchronize with the rendering thread,
				// set up texture ids only at initialization time.
				CameraTextureIds[i] = CameraTextures[i].GetNativeTextureID();
	...
	void Update()
	{
        ...
        CameraLeft.transform.position = 
			transform.position + Vector3.Scale( NeckPosition
                + cq * ( EyeCenterPosition + new Vector3( IPD * -0.5f, 0.0f, 0.0f ) ), transform.lossyScale );
	...
	{
        if (UsePlayerEyeHeight && (!PrevUsePlayerEyeHeight || UpdateCamerasDirtyFlag))
		{
